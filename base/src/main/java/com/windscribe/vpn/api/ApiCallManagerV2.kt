...
@Singleton
class ApiCallManagerV2 @Inject constructor(
    ...
    private fun createQueryMap(
        extraParams: Map<String, String>? = null, authRequired: Boolean = true
    ): Map<String, String> {
                ...
                access.dataClass?.let {
                    return@flatMap Single.fromCallable {
                        ...
                        appContext.preference.setStaticAccessIp(
                            PreferencesKeyConstants.ACCESS_API_IP_1, it.hosts[0]
                        )
                        appContext.preference.setStaticAccessIp(
                            PreferencesKeyConstants.ACCESS_API_IP_2, it.hosts[1]
                        )
                        listOf(
    ...
    private fun <T> responseToModel(
        responseBody: ResponseBody, modelType: Class<T>
    ...
    fun <T> call(
        ...
        try {
            ...
            return callOrSkip(
                apiCallType, service, DomainType.Primary, primaryDomain!!, protect, params
            ).onErrorResumeNext {
                ...
                if (it is HttpException && isErrorBodyValid(it)) {
                    ...
                    if (BuildConfig.DEV || BuildConfig.API_STATIC_CERT.isEmpty()) {
                        ...
                        if (domainFailOverManager.isAccessible(
                                DomainType.DirectIp1, apiCallType
                            )) {
                            domainFailOverManager.setDomainBlocked(
                ...
                if (it is HttpException && isErrorBodyValid(it)) {
                    ...
                    if (BuildConfig.DEV || BuildConfig.API_STATIC_CERT.isEmpty()) {
                        ...
                        if (domainFailOverManager.isAccessible(
                                DomainType.DirectIp2, apiCallType
                            )) {
                            domainFailOverManager.setDomainBlocked(
                                DomainType.DirectIp1, apiCallType
                            )
    ...
    override suspend fun getWebSession(extraParams: Map<String, String>?): Result<GenericResponseClass<WebSession?, ApiErrorResponse?>> {
        return call(extraParams, modelType = WebSession::class.java) { apiService, params, _ ->
            apiService.getWebSession(params)
        }.toResult()
    }

    override suspend fun addUserEmailAddress(extraParams: Map<String, String>?): Result<GenericResponseClass<AddEmailResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = AddEmailResponse::class.java
        ) { apiService, params, _ ->
            apiService.postUserEmailAddress(params)
        }.toResult()
    }

    override suspend fun checkConnectivityAndIpAddress(extraParams: Map<String, String>?): Result<GenericResponseClass<String?, ApiErrorResponse?>> {
        return call(
    ...
    override suspend fun claimAccount(extraParams: Map<String, String>?): Result<GenericResponseClass<ClaimAccountResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = ClaimAccountResponse::class.java
        ) { apiService, params, _ ->
            apiService.claimAccount(params)
        }.toResult()
    }

    override suspend fun getBestLocation(extraParams: Map<String, String>?): Result<GenericResponseClass<BestLocationResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = BestLocationResponse::class.java
        ) { apiService, params, _ ->
            apiService.getBestLocation(params)
        }.toResult()
    }

    override suspend fun getBillingPlans(extraParams: Map<String, String>?): Result<GenericResponseClass<BillingPlanResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = BillingPlanResponse::class.java
        ) { apiService, params, _ ->
            apiService.getBillingPlans(params)
        }.toResult()
    }

    override suspend fun getMyIp(extraParams: Map<String, String>?): Result<GenericResponseClass<GetMyIpResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = GetMyIpResponse::class.java
        ) { apiService, params, _ ->
            apiService.getMyIP(params)
        }.toResult()
    }

    override suspend fun getNotifications(extraParams: Map<String, String>?): Result<GenericResponseClass<NewsFeedNotification?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = NewsFeedNotification::class.java
        ) { apiService, params, _ ->
            apiService.getNotifications(params)
        }.toResult()
    }

    override suspend fun getPortMap(extraParams: Map<String, String>?): Result<GenericResponseClass<PortMapResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = PortMapResponse::class.java
        ) { apiService, params, _ ->
            apiService.getPortMaps(params, arrayOf("wstunnel"))
        }.toResult()
    }

    override suspend fun getReg(extraParams: Map<String, String>?): Result<GenericResponseClass<RegToken?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = RegToken::class.java
        ) { apiService, params, _ ->
            apiService.getReg(params)
        }.toResult()
    }

    override suspend fun getServerConfig(extraParams: Map<String, String>?): Result<GenericResponseClass<String?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = String::class.java
        ) { apiService, params, _ ->
            apiService.getServerConfig(params)
        }.toResult()
    }

    override suspend fun getServerCredentials(extraParams: Map<String, String>?): Result<GenericResponseClass<ServerCredentialsResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = ServerCredentialsResponse::class.java
        ) { apiService, params, _ ->
            apiService.getServerCredentials(params)
        }.toResult()
    }

    override suspend fun getServerCredentialsForIKev2(extraParams: Map<String, String>?): Result<GenericResponseClass<ServerCredentialsResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = ServerCredentialsResponse::class.java
        ) { apiService, params, _ ->
        ...
        return call(
            ...
            if (directIp) {
                apiService.getServerListDirectIp(
                    billingPlan, locHash, alcList, overriddenCountryCode
                )
            } else {
    ...
    override suspend fun getSessionGeneric(
        extraParams: Map<String, String>?, protect: Boolean
    ): Result<GenericResponseClass<UserSessionResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = UserSessionResponse::class.java, protect = protect
        ) { apiService, params, _ ->
            apiService.getSession(params)
        }.toResult()
    }

    override suspend fun getSessionGeneric(extraParams: Map<String, String>?): Result<GenericResponseClass<UserSessionResponse?, ApiErrorResponse?>> {
        return call(
            extraParams,
    ...
    override suspend fun getSessionGenericInConnectedState(extraParams: Map<String, String>?): Result<GenericResponseClass<UserSessionResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = UserSessionResponse::class.java
        ) { apiService, params, _ ->
            apiService.getSession(params)
        }.toResult()
    }

    override suspend fun getStaticIpList(extraParams: Map<String, String>?): Result<GenericResponseClass<StaticIPResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = StaticIPResponse::class.java
        ) { apiService, params, _ ->
            apiService.getStaticIPList(params)
        }.toResult()
    }

    override suspend fun logUserIn(extraParams: Map<String, String>?): Result<GenericResponseClass<UserLoginResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = UserLoginResponse::class.java
        ) { apiService, params, _ ->
            apiService.userLogin(params)
        }.toResult()
    }

    override suspend fun recordAppInstall(extraParams: Map<String, String>?): Result<GenericResponseClass<String?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = String::class.java
        ) { apiService, params, _ ->
    ...
    override suspend fun resendUserEmailAddress(extraParams: Map<String, String>?): Result<GenericResponseClass<AddEmailResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = AddEmailResponse::class.java
        ) { apiService, params, _ ->
    ...
    override suspend fun sendTicket(map: Map<String, String>?): Result<GenericResponseClass<TicketResponse?, ApiErrorResponse?>> {
        return call(
            map, modelType = TicketResponse::class.java
        ) { apiService, params, _ ->
            apiService.sendTicket(params)
        }.toResult()
    }

    override suspend fun signUserIn(extraParams: Map<String, String>?): Result<GenericResponseClass<UserRegistrationResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = UserRegistrationResponse::class.java
        ) { apiService, params, _ ->
            apiService.userRegistration(params)
        }.toResult()
    }

    override suspend fun verifyPurchaseReceipt(extraParams: Map<String, String>?): Result<GenericResponseClass<GenericSuccess?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = GenericSuccess::class.java
        ) { apiService, params, _ ->
            apiService.verifyPayment(params)
        }.toResult()
    }

    override suspend fun verifyExpressLoginCode(extraParams: Map<String, String>?): Result<GenericResponseClass<VerifyExpressLoginResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = VerifyExpressLoginResponse::class.java
        ) { apiService, params, _ ->
            apiService.verifyExpressLoginCode(params)
        }.toResult()
    }

    override suspend fun generateXPressLoginCode(
        extraParams: Map<String, String>?
    ): Result<GenericResponseClass<XPressLoginCodeResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = XPressLoginCodeResponse::class.java
        ) { apiService, params, _ ->
    ...
    override suspend fun verifyXPressLoginCode(
        extraParams: Map<String, String>?
    ): Result<GenericResponseClass<XPressLoginVerifyResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = XPressLoginVerifyResponse::class.java
        ) { apiService, params, _ ->
            apiService.verifyXPressLoginCode(params)
        }.toResult()
    }

    override suspend fun postDebugLog(extraParams: Map<String, String>?): Result<GenericResponseClass<GenericSuccess?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = GenericSuccess::class.java
        ) { apiService, params, _ ->
    ...
    override suspend fun postPromoPaymentConfirmation(extraParams: Map<String, String>?): Result<GenericResponseClass<GenericSuccess?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = GenericSuccess::class.java
        ) { apiService, params, _ ->
    ...
    override suspend fun updateRobertSettings(
        extraParams: Map<String, String>?
    ): Result<GenericResponseClass<GenericSuccess?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = GenericSuccess::class.java
        ) { apiService, params, _ ->
    ...
    override suspend fun syncRobert(
        extraParams: Map<String, String>?
    ): Result<GenericResponseClass<GenericSuccess?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = GenericSuccess::class.java
    ...
    override suspend fun getRobertSettings(
        extraParams: Map<String, String>?
    ): Result<GenericResponseClass<RobertSettingsResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = RobertSettingsResponse::class.java
        ) { apiService, params, _ ->
            apiService.getRobertSettings(params)
        }.toResult()
    }

    override suspend fun getRobertFilters(extraParams: Map<String, String>?): Result<GenericResponseClass<RobertFilterResponse?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = RobertFilterResponse::class.java
        ) { apiService, params, _ ->
            apiService.getRobertFilters(params)
        }.toResult()
    }

    override suspend fun deleteSession(extraParams: Map<String, String>?): Result<GenericResponseClass<GenericSuccess?, ApiErrorResponse?>> {
        return call(
            extraParams, modelType = GenericSuccess::class.java
        ) { apiService, params, _ ->
    ...
    override suspend fun getConnectedIp(): Result<GenericResponseClass<String?, ApiErrorResponse?>> {
