...
class DispatcherUsageMonitor(
    ...
    private fun sampleAndLog() {
        ...
        for ((thread, stack) in workers) {
            ...
            val top =
                stack.firstOrNull { it.className.startsWith("com.windscribe") }
                    ?: stack.firstOrNull()
            ...
            val isParked =
                top?.className?.contains("park", ignoreCase = true) == true ||
            ...
            val knownFrame =
                stack.firstNotNullOfOrNull { frame ->
                    KNOWN_LONG_RUNNING_FRAMES
                        .firstOrNull { (cls, method) ->
        ...
        val knownSummary =
            if (knownLongRunningCounts.isEmpty()) {
                ""
            } else {
    ...
    companion object {
        ...
        private val KNOWN_LONG_RUNNING_FRAMES: List<Pair<String, String>> =
            listOf(
