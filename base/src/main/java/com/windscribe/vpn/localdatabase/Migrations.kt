...
object Migrations {
    ...
    val migration_26_27: Migration =
        object : Migration(26, 27) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE City ADD COLUMN ovpn_x509 Text")
                invalidateData()
    ...
    val migration_27_28: Migration =
        object : Migration(27, 28) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE StaticRegion ADD COLUMN ovpnX509 Text")
                invalidateData()
    ...
    val migration_29_31: Migration =
        object : Migration(29, 31) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE City ADD COLUMN link_speed Text")
                database.execSQL("ALTER TABLE City ADD COLUMN health INTEGER NOT NULL DEFAULT 0")
                database.execSQL(
    ...
    val migration_33_34: Migration =
        object : Migration(33, 34) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE PingTime ADD COLUMN updated_at INTEGER NOT NULL DEFAULT 0")
                database.execSQL("ALTER TABLE PingTime ADD COLUMN ip Text")
                invalidateData()
    ...
    val migration_34_35: Migration =
        object : Migration(34, 35) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE StaticRegion ADD COLUMN status INTEGER")
                invalidateData()
    ...
    val migration_35_36: Migration =
        object : Migration(35, 36) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE Favourite ADD COLUMN pinned_ip TEXT")
                database.execSQL("ALTER TABLE Favourite ADD COLUMN pinned_node_ip TEXT")
                invalidateData()
    ...
    val migration_36_37: Migration =
        object : Migration(36, 37) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE StaticRegion ADD COLUMN gps TEXT")
                invalidateData()
    ...
    val migration_38_39: Migration =
        object : Migration(38, 39) {
            ...
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                invalidateData()
                logger.debug(
                    "Migrated database from version:38 to version:39 - V2 Complete (Server, Location (renamed from Region), Datacenter (renamed from City))",
                )
            }
        }
    ...
    val migration_39_40: Migration =
        object : Migration(39, 40) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE Server ADD COLUMN ipv6 INTEGER NOT NULL DEFAULT 0")
                invalidateData()
