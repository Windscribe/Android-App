...
class WgConfigRepository
    ...
    constructor(
        ...
        suspend fun getWgParams(
            ...
            val localParams =
                generateKeys(forceInit).getOrElse { error ->
            ...
            val userPublicKey =
                KeyPair(Key.fromBase64(localParams.privateKey))
                    .publicKey
                    .toBase64()
            ...
            val connectConfig =
                generateLanIpAddress(userPublicKey, localParams.hashedCIDR, localParams.hashedCIDRV6, supportsV6)
                    .getOrElse { return it }
        ...
        private suspend fun generateKeys(forceInit: Boolean): CallResult<WgLocalParams> {
            ...
            if (!forceInit &&
                cachedParams?.hashedCIDR != null &&
                cachedParams.hashedCIDRV6 != null &&
                isKeyRegisteredWithServer &&
                !skipCacheOnFirstInit
            ) {
            ...
            val keyPair =
                if (cachedParams != null && !forceInit) {
            ...
            val initResponse =
                performWgInitWithRetry(publicKey, forceInit)
                    .getOrElse { return it }
        ...
        private fun generateLanIpAddress(
            ...
            val cidr =
                hashedCIDR?.takeIf { it.isNotEmpty() }
            ...
            return runCatching {
                ...
                val (address, dns) =
                    if (shouldGenerateIPv6) {
                        val cidrV6 =
                            hashedCIDRv6?.takeIf { it.isNotEmpty() }
                        ...
                        logger.debug(
                            "Generated IPv4: $ipv4Address, IPv6: $ipv6WithCIDR (mode: $ipv6Mode, server supports IPv6: $supportsV6 $publicKey)",
                        )
...
data class WgRemoteParams(
    ...
    private fun String.maskSensitive(visibleChars: Int = 8): String = if (length > visibleChars) "***${takeLast(visibleChars)}" else "***"
}
