...
class IpRepositoryTest {
    ...
    private fun apiError(code: Int, message: String): GenericResponseClass<GetMyIpResponse?, ApiErrorResponse?> {
        ...
        ApiErrorResponse::class.java.declaredFields
            ...
            ?.apply { isAccessible = true; set(err, code) }
        ApiErrorResponse::class.java.declaredFields
            .firstOrNull { it.name.equals("errorMessage", ignoreCase = true) }
            ?.apply { isAccessible = true; set(err, message) }
    ...
    @Test
    fun `disconnected online success stores trimmed IP and emits Success`() = runTest {
    ...
    @Test
    fun `state starts as Loading before the 500ms delay elapses`() = runTest {
        connectionState.value = VPNState(VPNState.Status.Disconnected)
        isOnline.value = true
    ...
    @Test
    fun `disconnected API error falls back to stored IP`() = runTest {
        connectionState.value = VPNState(VPNState.Status.Disconnected)
        isOnline.value = true
    ...
    @Test
    fun `disconnected and offline uses stored IP without calling API`() = runTest {
        connectionState.value = VPNState(VPNState.Status.Disconnected)
    ...
    @Test
    fun `disconnected, offline, no stored IP emits Error`() = runTest {
        connectionState.value = VPNState(VPNState.Status.Disconnected)
        isOnline.value = false
    ...
    @Test
    fun `API exception treated as error and falls back to stored IP`() = runTest {
        connectionState.value = VPNState(VPNState.Status.Disconnected)
        isOnline.value = true
    ...
    @Test
    fun `connected branch loads from storage and skips API`() = runTest {
    ...
    @Test
    fun `connected with no stored IP emits Error`() = runTest {
        connectionState.value = VPNState(VPNState.Status.Connected)
        every { preferences.userIP } returns null
    ...
    @Test
    fun `vpn transition disconnected to connected switches to storage IP`() = runTest {
        connectionState.value = VPNState(VPNState.Status.Disconnected)
        isOnline.value = true
    ...
    @Test
    fun `vpn transition connected to disconnected refreshes from API after delay`() = runTest {
        connectionState.value = VPNState(VPNState.Status.Connected)
    ...
    @Test
    fun `mapLatest cancels in-flight disconnect refresh when a new vpn state arrives`() = runTest {
        connectionState.value = VPNState(VPNState.Status.Disconnected)
        isOnline.value = true
    ...
    @Test
    fun `update emits a refresh that retriggers the branch logic`() = runTest {
        connectionState.value = VPNState(VPNState.Status.Disconnected)
        isOnline.value = true
        coEvery { apiCallManager.getApiIp() } returnsMany listOf(
            ipResponse("1.1.1.1"),
            ipResponse("2.2.2.2")
        )
    ...
    @Test
    fun `long IPv6-shaped response has zero runs collapsed before storage`() = runTest {
        connectionState.value = VPNState(VPNState.Status.Disconnected)
        isOnline.value = true
