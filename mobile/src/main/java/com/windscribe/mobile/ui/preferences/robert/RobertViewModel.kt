...
abstract class RobertViewModel : ViewModel() {
    ...
    open fun onFilterSettingChanged(
        robertFilter: RobertFilter,
        status: Int,
    ) {}
...
class RobertViewModelImpl
    ...
    constructor(
        ...
        override fun onFilterSettingChanged(
            robertFilter: RobertFilter,
            status: Int,
        ) {
            viewModelScope.launch {
                ...
                when (result) {
                    is CallResult.Error -> {
                        _showProgress.value = false
                        _goToState.emit(
                            RobertGoToState.Toast(ToastMessage.Localized(com.windscribe.vpn.R.string.failed_to_update_robert_rules)),
                        )
                        loadRobertFilters()
                    }
                    ...
                    is CallResult.Success -> {
                        _showProgress.value = false
                        _goToState.emit(
                            RobertGoToState.Toast(ToastMessage.Localized(com.windscribe.vpn.R.string.successfully_updated_robert_rules)),
                        )
